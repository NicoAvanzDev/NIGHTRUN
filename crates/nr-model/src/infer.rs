...
//!
//! Weight matrices are dtype-tagged (Q1_0 / PQ2_0 / Q8_0 / Q4_K / Q6_K); dispatch happens
...
impl<'m> QMat<'m> {
    fn from_view(v: TensorView<'m>) -> Result<QMat<'m>, ParseError> {
        Ok(match v.dtype {
            ...
            TensorDtype::PQ2_0 => QMat::PQ2(v.pq2()),
    ...
    fn dequant_row(&self, r: usize, cols: usize, out: &mut [f32]) {
        match self {
            ...
            QMat::PQ2(blocks) => {
                let bpr = cols / PQK2_0;
                pq2::dequantize(&blocks[r * bpr..(r + 1) * bpr], out);
            }
            QMat::Q8(blocks) => {
                let bpr = cols / QK8_0;
...
impl Acts {
    ...
    fn matvec(&mut self, y: &mut [f32], w: QMat, x: &[f32]) {
        ...
        match w {
            ...
            QMat::PQ2(blocks) => {
                let xq = &mut self.q8[..cols / QK8_0];
                pq2::quantize_activations(x, xq);
                pq2::matvec(y, blocks, xq, rows, cols);
            }
            QMat::Q8(blocks) => {
                let xq = &mut self.q8[..cols / QK8_0];
                q8::quantize(x, xq);
    ...
    fn matmul(&mut self, y: &mut [f32], w: QMat, xs: &[f32], batch: usize) {
        ...
        match w {
            ...
            QMat::PQ2(blocks) => {
                ...
                let xq = &mut self.q8[..batch * bpr];
                for b in 0..batch {
                    pq2::quantize_activations(
                        &xs[b * cols..(b + 1) * cols],
                        &mut xq[b * bpr..(b + 1) * bpr],
                    );
                }
                pq2::matmul(y, blocks, xq, rows, cols, batch);
            }
            QMat::Q8(blocks) => {
                let bpr = cols / QK8_0;
                let xq = &mut self.q8[..batch * bpr];
        ...
        for (y, w) in jobs.iter_mut() {
            ...
            match w {
                ...
                QMat::PQ2(blocks) => {
                    ...
                    pq2::matmul(y, blocks, &self.q8[..batch * bpr], rows, cols, batch);
                }
                QMat::Q8(blocks) => {
                    let bpr = cols / QK8_0;
                    if q8_for_bonsai != Some(false) {
                        for b in 0..batch {
                            q8::quantize(
                                &xs[b * cols..(b + 1) * cols],
                                &mut self.q8[b * bpr..(b + 1) * bpr],
    ...
    fn matvec_group(&mut self, jobs: &mut [(&mut [f32], QMat)], x: &[f32]) {
        ...
        for (y, w) in jobs.iter_mut() {
            match w {
                ...
                QMat::PQ2(blocks) => {
                    if q8_for_bonsai != Some(true) {
                        pq2::quantize_activations(x, &mut self.q8[..cols / QK8_0]);
                        q8_for_bonsai = Some(true);
                    }
                    pq2::matvec(y, blocks, &self.q8[..cols / QK8_0], y.len(), cols);
                }
                QMat::Q8(blocks) => {
                    if q8_for_bonsai != Some(false) {
