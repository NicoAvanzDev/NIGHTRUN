...
fn logo(surf: &mut Surface, fonts: &Fonts) {
    ...
    draw::text_glow(surf, font, x, y, LOGO, scale, tracking, shear, theme::NEON_MAGENTA, 4 * scale as i32);
    // Drop shadow.
    let sh = 3 * scale as i32;
    draw::text_fx(surf, font, x + sh, y + sh, LOGO, scale, tracking, shear, |_, _| 0x14042a, 230);
    // Gradient fill with a darkened "chrome" band around 55% height.
    draw::text_fx(surf, font, x, y, LOGO, scale, tracking, shear, |row, rows| {
    ...
    draw::text_glow(surf, &fonts.head, tx, ty, TAGLINE, tag_scale, tag_tracking, 0, theme::NEON_CYAN, 2);
    draw::text(surf, &fonts.head, tx, ty, TAGLINE, 0xccf9ff, tag_scale, tag_tracking);
    ...
    draw::line(surf, tx + tt + gap, rule_y, x + tw, rule_y, theme::NEON_CYAN, 110);
}
...
fn footer(surf: &mut Surface, fonts: &Fonts, status: &str) {
    ...
    draw::text(surf, &fonts.small, (w - tw) / 2, h - 28, status, theme::TEXT_DIM, 1, 1);
}
