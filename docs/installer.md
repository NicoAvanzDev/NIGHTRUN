...
- not the disk backing `/`, `/boot`, `/home`, or active swap;
...
printed. Ctrl+C mid-write prints an honest "media is INCOMPLETE" message
...
- `scripts/installer/tests/run.sh` — 42 checks, no root, and no contact
  ...
  validation, GGUF magic, rendering with and without color.
